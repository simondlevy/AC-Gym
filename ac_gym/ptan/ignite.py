...
class EndOfEpisodeHandler:
    def __init__(self,
        ...
        :param subsample_end_of_episode: if given, end of episode event will
        be subsampled by this amount '''
    ...
    def __call__(self, engine: Engine):
        for reward, steps in self._exp_source.pop_rewards_steps():
            ...
            if (self._subsample_end_of_episode is None or
               engine.state.episode % self._subsample_end_of_episode == 0):
                engine.fire_event(EpisodeEvents.EPISODE_COMPLETED)
            if (self._bound_avg_reward is not None and
               engine.state.metrics['avg_reward'] >= self._bound_avg_reward):
    ...
    def _update_smoothed_metrics(self,
                                 ...
                                 reward: float,
                                 steps: int):
        for attr_name, val in zip(('avg_reward', 'avg_steps'),
                                  (reward, steps)):
...
class EpisodeFPSHandler:
    ...
    def attach(self, engine: Engine, manual_step: bool = False):
        self._timer.attach(engine,
                           step=(None if manual_step
                                 else Events.ITERATION_COMPLETED))
        engine.add_event_handler(EpisodeEvents.EPISODE_COMPLETED, self)

    def step(self):
        '''
        If manual_step=True on attach(), this method should be used every time
        we've communicated with environment to get proper FPS
        :return:
        '''
        self._timer.step()
    ...
    def __call__(self, engine: Engine):
        ...
        engine.state.metrics[self.TIME_PASSED_METRIC] = (time.time() -
                                                         self._started_ts)
        self._timer.reset()
...
class PeriodicEvents:
    '''
    The same as CustomPeriodicEvent from ignite.contrib, but use true amount
    of iterations, which is good for TensorBoard
    '''
